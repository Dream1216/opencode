...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* events.project(SessionEvent.ModelInvocation.Attempted, () => Effect.void)
    yield* events.project(SessionEvent.ModelInvocation.Retried, () => Effect.void)
    yield* events.project(SessionEvent.ModelInvocation.Usage, () => Effect.void)
    yield* events.project(SessionEvent.ModelInvocation.Completed, () => Effect.void)
    yield* events.project(SessionEvent.ModelInvocation.Failed, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Scheduled, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Resumed, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Started, () => Effect.void)
    yield* events.project(SessionEvent.Worker.StopRequested, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Stopped, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Completed, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Failed, () => Effect.void)
    yield* events.project(SessionEvent.Worker.Lease, () => Effect.void)
    yield* events.project(SessionEvent.Workflow.Bound, () => Effect.void)
    yield* events.project(SessionEvent.Tenant.Bound, () => Effect.void)
    yield* events.project(SessionEvent.Rls.Evaluated, () => Effect.void)
    yield* events.project(SessionEvent.Audit.Recorded, () => Effect.void)
    yield* events.project(SessionEvent.ReleaseGovernance.Evaluated, () => Effect.void)
